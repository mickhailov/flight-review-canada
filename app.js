...
function questionView(step){
  ...
  const gradeSummaries={pass:'Meets the requirement',minor:'Small correction needed',major:'Aim not fully achieved',critical:'Unsafe or invalid'};
  return `<p class="question-kicker">${step.suggested?'Suggested reviewer question':'Assessment observation'} · ${s.title}</p><h1 class="question-title">${step.text}</h1>${step.note?`<p class="question-note">${step.note}</p>`:''}${step.demo?maneuverDemo(step.demo):''}<details class="official-answer" open><summary>Transport Canada expected answer / evidence</summary><p>${step.answer}</p><div class="source-citation"><span><small>Pinpoint authority</small><b>${step.source.reference||step.source.label}</b><em>${step.source.label}</em></span><a href="${step.source.url}" target="_blank" rel="noreferrer" aria-label="Open ${step.source.reference||step.source.label} in a new tab">Open exact section ↗</a></div></details><h2 class="criteria-title">Select the performance level</h2><div class="criteria-grid">${['pass','minor','major','critical'].map(g=>`<button class="criterion ${state.grades[state.step]===g?'selected':''}" data-grade="${g}"><strong>${g}</strong><p>${gradeSummaries[g]}</p></button>`).join('')}</div><label class="notes-label" for="notes">Reviewer notes</label><textarea id="notes" placeholder="Record the observed evidence...">${state.notes[state.step]||''}</textarea>`;
}
